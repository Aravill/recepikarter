import { describe, expect, it } from 'vitest'
import { CATEGORIES, DIFFICULTIES } from '../../../shared/types/recipe'
import { PROMPT_LANGS, buildImportPrompt } from '../../../shared/utils/import-prompt'
import { parseRecipeInput } from '../../../server/utils/validate'

const EXAMPLE_HEADING: Record<(typeof PROMPT_LANGS)[number], string> = {
  en: 'EXAMPLE OUTPUT',
  cs: 'PŘÍKLAD VÝSTUPU',
}

describe.each(PROMPT_LANGS)('buildImportPrompt(%s)', (lang) => {
  const prompt = buildImportPrompt(lang)

  it('lists every allowed category and difficulty as a quoted value', () => {
    for (const category of CATEGORIES) expect(prompt).toContain(`"${category}"`)
    for (const difficulty of DIFFICULTIES) expect(prompt).toContain(`"${difficulty}"`)
  })

  it('embeds an example the import endpoint would accept', () => {
    const block = prompt.slice(prompt.indexOf(EXAMPLE_HEADING[lang]))
    const example = JSON.parse(block.slice(block.indexOf('['), block.lastIndexOf(']') + 1))
    expect(Array.isArray(example)).toBe(true)
    expect(example).toHaveLength(1)
    expect(() => parseRecipeInput(example[0])).not.toThrow()
  })

  it('asks for metric units, including cups of dry goods as grams', () => {
    expect(prompt).toContain('1 cup of flour')
    expect(prompt).toContain('125 g mouky')
    expect(prompt).toMatch(/\bml\b/)
    expect(prompt).toMatch(/\bkg\b/)
  })

  it('ends where the user pastes their recipe', () => {
    expect(prompt.trimEnd()).toMatch(/:$/)
  })
})

it('defaults to English', () => {
  expect(buildImportPrompt()).toBe(buildImportPrompt('en'))
})
