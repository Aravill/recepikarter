...
- **Recipe photos are files, not rows.** `server/utils/photos.ts` re-encodes every upload with `sharp` into `data/photos/<id>-<ms>.webp` + `-thumb.webp`; the DB only stores the basename (`photo_file`). Anything that removes a recipe must also call `removePhotoFiles`, and the filename changes on every upload on purpose — the photo route sends `Cache-Control: immutable`, so the `?v=<file>` in the URL is what busts the cache. `sharp`'s prebuilt binaries don't decode HEIC; the form only accepts JPEG/PNG/WebP (iOS converts HEIC for `image/*` inputs).
...
- **A worktree-isolated background agent lands at `.claude/worktrees/<id>/`, inside this repo's own tree.** It's gitignored, but `eslint.config.mjs` and `vitest.config.ts` both have explicit `ignores`/`exclude` entries for `.claude/worktrees/**` too — without those, `npm run lint`/`npm run test` in the main checkout recurse into any concurrently-running worktree agent's copy (ESLint hard-fails on its mid-setup config; Vitest silently runs its tests too, inflating the count). Keep those entries if either config gets rewritten.
...
- Env vars: `AUTH_USERNAME`, `AUTH_PASSWORD_HASH` (see gotcha above), `NUXT_SESSION_PASSWORD` are required — see `.env.example`. `RECIPE_DB_PATH` is an optional override for the SQLite file location (defaults to `./data/recipes.sqlite`), not listed in `.env.example`; `RECIPE_PHOTOS_DIR` does the same for uploaded recipe photos (defaults to `./data/photos`).
